(ns workday.main
  "Kotodama WASM entrypoint for the Workday clean-room actor (L4) — Clojure port.
  ...
  facts (`workday.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "workday")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"workday.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "workday-compat" :tier tier :entities entities} 200])
...
(defn start! [] :workday-compat/ready)
